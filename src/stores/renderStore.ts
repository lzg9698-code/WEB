...
import { renderApi, type RenderFile, type RenderResult, type ApiResponse } from '@/services/api'
...
export const useRenderStore = defineStore('render', () => {
  ...
  const renderFiles = computed(() => renderResult.value?.files || [])
  ...
  const startRender = async (
    ...
    try {
      ...
      if (response.data.success) {
        ...
        console.log(`✅ 渲染成功: ${Object.keys(result.files || {}).length} 个文件`)
