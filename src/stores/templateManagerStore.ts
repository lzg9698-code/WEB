...
export const useTemplateManagerStore = defineStore("templateManager", () => {
  ...
  const exportPackage = async (packageName: string): Promise<boolean> => {
    ...
    try {
      const blob = await templateApi.exportTemplate(packageName);
  ...
  const createPackage = async (data: {
    ...
    console.log("🔒 创建模板包 - 约束机制已激活");
    console.log(`📋 模板包: ${data.displayName}`);
    ...
    try {
      ...
      if (response.data.success) {
        console.log(`✅ 成功创建模板包: ${response.data.data.displayName}`);
        ...
        error.value = response.data.error || "创建模板包失败";
        console.error("❌ 创建模板包失败:", error.value);
        return false;
      }
    } catch (err) {
      error.value = err instanceof Error ? err.message : "未知错误";
      console.error("❌ 创建模板包异常:", err);
      return false;
    } finally {
      loading.value = false;
    }
  ...
  const duplicatePackage = async (
    sourceName: string,
    data: { newName?: string; newDisplayName?: string },
  ): Promise<boolean> => {
    console.log("🔒 复制模板包 - 约束机制已激活");
    console.log(`📋 源模板包: ${sourceName}`);

    loading.value = true;
    error.value = null;

    try {
      const response = await templateApi.duplicateTemplate(sourceName, data);

      if (response.data.success) {
        console.log(`✅ 成功复制模板包: ${response.data.data.displayName}`);
        ...
        error.value = response.data.error || "复制模板包失败";
        console.error("❌ 复制模板包失败:", error.value);
        return false;
      }
    } catch (err) {
      error.value = err instanceof Error ? err.message : "未知错误";
      console.error("❌ 复制模板包异常:", err);
      return false;
    } finally {
      loading.value = false;
    }
  ...
  // 获取模板预览
  const getPreview = async (packageName: string): Promise<string> => {
    console.log("🔒 获取模板预览 - 约束机制已激活");

    try {
      const response = await templateApi.getTemplatePreview(packageName);
