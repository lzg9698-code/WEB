...
const localValidation = computed(() => props.validation || { valid: true, errors: {}, warnings: {} })
...
const hasDefaults = computed(() => {
  return Object.values(props.group.parameters).some(param => param.default !== undefined)
})
...
const getWarningCount = (parameters: any[]) => {
  return parameters.filter(param => 
    localValidation.value.warnings[param.key]
  ).length
}

const getErrorCount = (parameters: any[]) => {
  return parameters.filter(param => 
    localValidation.value.errors[param.key]
  ).length
}
...
const handleParameterChange = (key: string, value: any, parameter: any) => {
  ...
  emit('change', props.group.key, { ...props.modelValue, [key]: value }, localValidation.value)
}
...
const shouldCollapse = computed(() => {
  ...
  const requiredParams = Object.values(props.group.parameters).filter(p => p.required)
...
// 监听是否应该折叠
watch(() => shouldCollapse.value, (should) => {
