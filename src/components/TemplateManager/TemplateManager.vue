  ...
  <div class="template-manager">
    <div class="manager-header">
      ...
      <div class="header-actions">
        ...
        <el-button @click="importTemplatePackage"> 📥 导入模板包 </el-button>
        <el-button @click="exportAllTemplates"> 📤 导出全部 </el-button>
      </div>
    </div>
...
const createRules: FormRules = {
  name: [
    { required: true, message: "请输入模板包名称", trigger: "blur" },
    {
      ...
      message: "名称必须以字母开头，只能包含字母、数字、下划线和横线",
      trigger: "blur",
    },
  ],
...
// 方法
const handleTemplateSelect = async (template: TemplatePackage | null) => {
  if (!template) return;
  selectedTemplate.value = template;
  console.log(`🔒 选择模板包: ${template.displayName}`);

  // 更新 store 中的当前模板
  await templateStore.selectPackage(template.name);
};
...
const confirmCreate = async () => {
  ...
  try {
    ...
    const success = await templateStore.createPackage({
    ...
    if (success) {
      ElMessage.success(`模板包 "${createForm.value.displayName}" 创建成功`);
      createDialogVisible.value = false;
    } else {
      ElMessage.error(templateStore.error || "创建模板包失败");
    }
  } catch (error) {
...
const exportAllTemplates = async () => {
  ...
  try {
    ...
    const templateToExport =
      selectedTemplate.value || templateStore.packages[0];
    if (templateToExport) {
